...
class PerturbationSolver:
    ...
    def compute_steady_state_numerically(
        ...
        # FIX: Use 1.0 for technology shock instead of 0.0 to avoid division by zero
        ss_values[states[1]] = 1.0  # Technology shock (a)
        
        # Set other shock steady states to zero
        ...
        # FIX: Add robust handling for complex numbers
        def eval_equations(values_dict):
            results = []
            for eq in equations:
                try:
                    # Substitute values and convert to float
                    result = eq.subs(values_dict)
                    # Check if result is complex and get real part if small imaginary component
                    if isinstance(result, complex) or hasattr(result, 'as_real_imag'):
                        if hasattr(result, 'as_real_imag'):
                            re, im = result.as_real_imag()
                            if abs(float(im)) < 1e-10:  # Small imaginary part, safe to take real part
                                result = float(re)
                            else:
                                logger.warning(f"Equation yielding significant complex part: {eq} = {result}")
                                result = float(re)  # Still proceed with real part, but warned
                        else:
                            if abs(result.imag) < 1e-10:  # Small imaginary part, safe to take real part
                                result = result.real
                            else:
                                logger.warning(f"Equation yielding significant complex part: {eq} = {result}")
                                result = result.real  # Still proceed with real part, but warned
                    else:
                        result = float(result)
                    
                    results.append(result)
                except Exception as e:
                    logger.error(f"Error evaluating equation: {eq}: {e}")
        ...
        if max(abs(np.array(residuals))) > 1e-6:
            ...
            # Solve for steady state, with more robust settings
            result = root(objective, x0, method='lm', options={'ftol': 1e-6, 'xtol': 1e-6})
            ...
            if result.success:
                ...
                logger.error(f"Failed to solve for steady state numerically: {result.message}")
                logger.info("Using initial values as approximation.")
                
                # We'll still return the best approximation we have
    ...
    def linearize_model(
        ...
        # Compute derivatives with better error handling
        for i, eq in enumerate(equations):
            # Derivatives w.r.t. current states
            for j, var in enumerate(states):
                try:
                    derivative = sp.diff(eq, var).subs(ss_values)
                    # Handle complex numbers
                    if isinstance(derivative, complex) or hasattr(derivative, 'as_real_imag'):
                        if hasattr(derivative, 'as_real_imag'):
                            re, im = derivative.as_real_imag()
                            if abs(float(im)) < 1e-10:
                                derivative = float(re)
                            else:
                                logger.warning(f"Derivative yielding complex result: d(eq{i+1})/d{var} = {derivative}")
                                derivative = float(re)
                        else:
                            if abs(derivative.imag) < 1e-10:
                    ...
                    logger.error(f"Error computing derivative d(eq{i+1})/d{var}: {e}")
                    A[i, j] = 0.0
